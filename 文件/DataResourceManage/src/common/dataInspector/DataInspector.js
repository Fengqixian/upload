...
Inspector.prototype.inspectObject = function(object) {
  ...
  for (var k in declaredProperties) {
    ...
    if (data[k] !== undefined) defaultValue = data[k]["meta_value"]=== undefined?data[k]:data[k]["meta_value"];
  ...
  if (this.includesOwnProperties) {
    for (var k in data) {
      ...
      tbody.appendChild(this.buildPropertyRow(k, (data[k]["meta_value"] === undefined ?(data[k]||""):data[k]["meta_value"])));
    }
  }
...
Inspector.prototype.buildPropertyRow = function(propertyName, propertyValue) {
  ...
  td1.textContent = (this.declaredProperties[propertyName]||{}).display||(propertyName.startsWith("_self_")?propertyName.replace("_self_",""):propertyName);
  tr.appendChild(td1);
...
Inspector.prototype.updateAllHTML = function() {
  ...
  if (!data) {  // clear out all of the fields
    ...
    for (var name in inspectedProps) {
      var input = inspectedProps[name];
      var propertyValue = (data[name]===undefined||data[name]["meta_value"]===undefined)? data[name]: data[name]["meta_value"];
      if (input instanceof HTMLSelectElement) {
...
Inspector.prototype.updateSelect = function(decProp, select, propertyName, propertyValue) {
  ...
  for (var i = 0; i < choices.length; i++) {
    ...
    opt.text = this.convertToString((choice.name||choice));
    opt.value = this.convertToString((choice.code||choice));
    select.add(opt, null);
  }
...
Inspector.prototype.updateAllProperties = function() {
  ...
  for (var name in inspectedProps) {
    ...
    if (type === "") {
      var oldval = data[name]["meta_value"]===undefined?data[name]:data[name]["meta_value"];
    ...
    switch (type) {
      ...
      case "select": value = (decProp.choicesArray[input.selectedIndex].code||decProp.choicesArray[input.selectedIndex]); break;
    }
    ...
    if(newValue["meta_value"]===undefined){
      newValue = value;
    }else{
      newValue["meta_value"]=value;
    }
    diagram.model.setDataProperty(data, name, newValue);
    //如果名称是***_name 那么更新name属性
    if(name.toLowerCase().endsWith("_name")){
      diagram.model.setDataProperty(data, "name", value);
    }
    // notify any listener
    if (this.propertyModified !== null) this.propertyModified(name, value, this);
  }
  diagram.model.setDataProperty(data, "isUpdated", true);
