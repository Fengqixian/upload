...
class Line {
  ...
  * */
  ...
  ball() {
    if (isIE()) {
      const radom = `ball_${this.linkInfo.from}_${this.linkInfo.to}`;
      ...
      style.type = 'text/css';
      var keyFrames = `
    @keyframes ball-run-${radom} {
      0% {
        left: ${this.data[0].x}px;
        top: ${this.data[0].y -2}px;
      }
      100% {
        left: ${this.data[1].x}px;
        top: ${this.data[1].y -2}px;
      }
    }
`;
      style.innerHTML = keyFrames;
      document.getElementsByTagName('head')[0].appendChild(style);
      this.container.select(`#${radom}`)
        .attr('style', `animation:ball-run-${radom} ${this.runTime()}s infinite linear;width:${this.ballRadius * 2}px;height:${this.ballRadius * 2}px;`);
    } else {
