<template>
    <div class="relation cb-relation-wrapper">
        <div ref="relation" style="width: 100%;height: 100%;"></div>
...
<script lang="ts">
    ...
    export default class Relation extends Vue {
        ...
        mounted() {
            ...
            this.headerId = this.arg.resourceId;
        ...
        getRelationData() {
            if (this.detail) {
                ...
                let params = {
                    resourceId: this.arg.etlTargetId,
                    nameEn: this.arg.nameEn,
                    etlId:this.arg.etlId
                };
                console.log(this.arg)
                this.$http.get(url, {params}).then(response => {
                    if (response.data.code === 0) {
                        if (response.data.code === 0) {
                            const {linkDataArray, nodeDataArray} = response.data.data;
                            this.linkDataArray = linkDataArray;
                            this.nodeDataArray = nodeDataArray;
                            if (nodeDataArray.length) {
                                this.headerId = this.nodeDataArray[0]['id'];
                                this.renderRelation();
                            }
                        }
                    }
        ...
        public renderRelation(): void {
            ...
            const {width, height} = this.$refs.relation['getBoundingClientRect']();
            this.width = width;
            this.height = height;
            const svg = d3.select(this.$refs.relation)
                .append('svg')
            ...
            const force = d3.layout.force()
                ...
                .size([this.width, this.height])                // 设置作用范围
            ...
            let gNodeEnter = gNode.enter()
                ...
                .on('mouseup', (d) => {
                    ...
                    this.linkClick && this.linkClick(d);
