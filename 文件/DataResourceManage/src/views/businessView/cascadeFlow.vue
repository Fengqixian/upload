<template>
    <div class="cascade-flow">
            ...
            <ul class="flow">
                <template v-for="(item) in flowData">
                    <li class="flow-item" :style="`background-color:${item.color}`"
                        ...
                        <i class="icon-bianji iconfont" @click.stop.prevent="editClick(item)"></i>
        ...
        <cb-card-relation v-model="cbCardRelationData" viewType="business"/>
...
<script lang="ts">
    ...
    export default class CascadeFlow extends Vue {
         ...
         * 血缘关系
         */
        relationVisible: boolean = false;
        relationTile: string = '血缘';
        relationHeight: number = 0;
        relationWidth: number = 0;
        relationData: object = {};
        ...
        infoRelationRelation: boolean = false;
        infoRelationWidth: number = 0;
        infoRelationHeight: number = 0;
        infoRelationData: object = {};
        infoRelationResourceId: string = '';
        infoRelationTableData: Array<any> = [];
        infoRelationInfoData: object = {};
        // ETL的
        infoRelationETLVisible: boolean = false;
        infoRelationETLWidth: number = 0;
        infoRelationETLHeight: number = 0;
        infoRelationETLData: object = {};
        infoRelationETLId: string = '';


        /**
         * 点击ETL节点
         */
        infoRelationNodeClick(data) {
            if (data.type !== 'ETL') return;
            this.infoRelationETLVisible = false;
            this.infoRelationETLId = data.id;
            let url = config.port('metaDataLineage') + '/getDataLineageETLDetail';
            let params = {
                resourceId: data.etlTargetId,
                nameEn: data.nameEn
            };
            this.$http.get(url, {params}).then(response => {
                ...
                setTimeout(() => {
                    const {width, height} = this.$refs.relationETLWrapper['getBoundingClientRect']();
                    this.infoRelationETLHeight = height;
                    this.infoRelationETLWidth = width;
                    this.infoRelationETLData = response.data.data;
                }, 10)
            });
        }
        ...
        getDataMetaTableData() {
            let params = {
                ...
                category: this.breadcrumb[this.breadcrumb.length - 1].nodeType,
