          ...
          <template slot="content" v-if="serverTreeActiveData !== ''">
            ...
            <div class="table-wrapper" v-if="SQLMode === '拖拽拼接SQL'">
              <div class="svg_wrapper">
                <svg width="100%" height="500px" id="server_svg">
                </svg>
              </div>
              <div class="table">
                <el-table
    ...
    <el-dialog :title="serverForm.id?'修改'+ serverForm.nameCn:'新增服务'"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary"
                   @click="submitServer(serverForm?'edit': 'add',serverForm)">{{serverForm.id?'修 改': '确 定'}}
        </el-button>
      </div>
    </el-dialog>
  </div>
...
<script lang="ts">
  ...
  export default class Index extends Vue {
    ...
    /*=============---------------可视化拖拽区域代码开始------------==========================*/
    public server_dom: any = null;  //  dom 被拖进的区域 这里是svg
    public server_el: any = null;   // 存放拖拽时候的鼠标事件相关信息
    public server_cbDragDom: any = null; // 存放从tree树拖出来的圆圈
    // 将数据库推拽到svg上
    public dragTableToSvg(dom, el, cbDragDom?): void {
      this.server_dom = dom;
      this.server_el = el;
      this.server_cbDragDom = cbDragDom;
      // this.middleFun = () => {
      //   this.dragDataToSvg(dom, el);
      ...
      if (dom !== null) {
        ...
        if (this.tableBoxPosition(dom, el)) {
          ...
          let flag = true;
          setTimeout(_ => {
            cbDragDom.addEventListener('mouseup', this.dragDataToSvg, true);
            this.dropTable = dom
          }, 1)
        } else {
          dom.removeAttribute('style');
          cbDragDom.removeEventListener('mouseup', this.dragDataToSvg, false)
          this.dropTable = ''
        }
      }
    }

    // 将数据形状添加到svg dom 被拖进的区域 这里是svg el为鼠标点击的相关信息
    public dragDataToSvg(): void {
      const dom = this.server_dom;
      const el = this.server_el;
      // 第一步： 获取svg右上角在页面的坐标
      const svg: object = dom.getBoundingClientRect();
      const svg_x: number = svg['x'] || svg['left'];
      const svg_y: number = svg['y'] || svg['top'];

      // 第二步： 获取鼠标的位置
      const mouse_x = el['clientX'];
      const mouse_y = el['clientY'];
      // 第三步：数据相对svg左上角坐标即svg上表的坐标
      const rect_x = mouse_x - svg_x - 25;
      const rect_y = mouse_y - svg_y - 25;
      // 创建table的形状并插入到svg上
      const rect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
      rect.setAttribute('x', rect_x.toString());
      rect.setAttribute('y', rect_y.toString());
    ...
    public handleDrag(node, nodeData, cbDragDom, el): void {
      ...
      const svgWrapper: any = document.querySelector('#server_svg');
      ...
      this.dragTableToSvg(svgWrapper, el, cbDragDom);
...
<style lang="less">
  .data-service {
    ...
    .table-wrapper {
      ...
      .column, .reference, .table, #server_svg {
        transition: all 0.2s;
      }
    }
  }
