...
<script lang="ts">
    ...
    @Component({})
    export default class DragRelation extends Vue {
        name: string = 'DragRelation';
        ...
        public mounted(): void {
            this.$nextTick(() => {
                // this.initSvg();
                const dragRelationSvg = new DragRelationClass({
                    nodes: this.nodes,
                    links: this.links,
                    width: 200,
                    height: 200,
                    containerClassName: '.drag-relation',
                    d3: d3,
                })
                dragRelationSvg.render();
        ...
        public initSvg(): void {
            ...
            var zoom = d3.behavior.zoom().scaleExtent([0, 20]).on("zoom", zoomed);

            function zoomed() {
                // 获取class
                const targetClassName = d3.event['sourceEvent'].target.getAttribute('class');
                if (targetClassName === 'node') {
                    // svg.attr("transform", `scale(${zoom.scale(10)})`);
                } else {
            ...
            var svg = d3.select('.drag-relation')
                .call(zoom)
                .append('svg')
                .attr('width', width)
                .attr('height', height);
            ...
            var nodes: Array<object> = this.nodes,
            ...
            function restart() {
                ...
                circle.selectAll('circle')
                    .style('fill', function (d) {
                        return (d === selected_node) ? d3['rgb'](colors(d.id)).darker().toString() : "#0e183fe6";
                    })
                ...
                g.append('svg:circle')
                    ...
                    .attr('r', 12)
                    .style('fill', function (d) {
                        return (d === selected_node) ? d3['rgb'](colors(d.id)).brighter().toString() : "#0e183fe6";
                    })
