...
<script lang="ts">
    ...
    export default class Relation extends Vue {
        ...
        public async getinfoData(data): Promise<void> {
            this.showInfo = false;
            let attrs: Array<object> = await this.getDetailAttr(data);
            let values: object = await this.getDetailValue(data);
            this.infoData = [...JSON.parse(JSON.stringify(this.baseInfoData))];
            attrs.forEach(attr => {
                this.infoData.push({
                    label: attr['nameCn'], // 展示的名称
                    value: '', // 展示的值
                    key: attr['mappingField'] // 对应的字段
                })
            });
        ...
        public renderRelation(): void {
            ...
            const svg = d3.select(`.md-relation-${this.id}`)
                ...
                .on('click', () => {
                    ...
                    if (this.isGetInfoData) this.isGetInfoData = false;
                })
            ...
            this.nodeDataArray.forEach(node => {
                if (node['id'] === _this.headerId) {
                    node['fixed'] = true;
                    node['px'] = _this.width / 3;
                    node['py'] = _this.height / 3;
                }
            });
