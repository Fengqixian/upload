...
import com.clinbrain.bd.shardingproxy.backend.communication.jdbc.recognizer.JDBCDriverURLRecognizerEngine;
...
public final class LogicSchemas {
    ...
    private void initSchemas(final Collection<String> localSchemaNames, 
        ...
        if (schemaRules.isEmpty()) {
            for(String key:schemaDataSources.keySet()){
                logicSchemas.put(key, createLogicSchema(key, schemaDataSources, null, isUsingRegistry));
            }
        }
