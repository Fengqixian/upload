...
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.command.admin.PostgreSQLUnsupportedCommandPacket;
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.command.query.binary.bind.PostgreSQLComBindPacket;
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.command.query.binary.describe.PostgreSQLComDescribePacket;
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.command.query.binary.execute.PostgreSQLComExecutePacket;
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.command.query.binary.parse.PostgreSQLComParsePacket;
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.command.query.binary.sync.PostgreSQLComSyncPacket;
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.command.query.text.PostgreSQLComQueryPacket;
import com.clinbrain.bd.shardingproxy.transport.postgresql.packet.generic.PostgreSQLComTerminationPacket;
...
public final class PostgreSQLCommandPacketFactory {
    ...
    public static PostgreSQLCommandPacket newInstance(
            final PostgreSQLCommandPacketType commandPacketType, final PostgreSQLPacketPayload payload, final int connectionId, final String username) throws SQLException {
        switch (commandPacketType) {
            case QUERY:
                return new PostgreSQLComQueryPacket(payload);
            case PARSE:
                return new PostgreSQLComParsePacket(payload,username);
            case BIND:
                return new PostgreSQLComBindPacket(payload, connectionId,username);
            case DESCRIBE:
