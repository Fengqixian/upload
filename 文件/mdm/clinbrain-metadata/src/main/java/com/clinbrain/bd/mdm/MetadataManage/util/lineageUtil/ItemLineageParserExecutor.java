...
public class ItemLineageParserExecutor extends AbstractLineageParserExecutor<LineageMetaValue> {
    ...
    void doParse(LineageMetaValue lineageValue) throws Exception {
        if(!MetaModuleConst.ITEM_MODULE_TYPE.equalsIgnoreCase(lineageValue.getDataType())&&!MetaModuleConst.SET_MODULE_TYPE.equalsIgnoreCase(lineageValue.getDataType())) return;
        ...
        if("business".equalsIgnoreCase(lineageValue.getViewType())){
            ...
            lineageValues.stream().filter(e ->!enviroment.parsedObjects.contains(e.getResourceId()))
                    .forEach(e ->{
                        ...
                        LineageNode tableNode = transferMetadataValue2Node(e);
